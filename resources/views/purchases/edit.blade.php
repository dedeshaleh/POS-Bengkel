...
@section('subtitle', 'Edit PO draft - ' . $purchase->invoice_number)
...
<section class="panel">
    ...
    <form method="post" action="{{ route('purchases.update', $purchase) }}" id="purchaseForm">
        ...
        @method('put')
        <div class="form-grid">
            <label>Invoice Number <input name="invoice_number" value="{{ old('invoice_number', $purchase->invoice_number) }}" required></label>
            <label>Purchase Date <input type="date" name="purchase_date" value="{{ old('purchase_date', $purchase->purchase_date->toDateString()) }}" required></label>
            <label>Supplier
                <input type="hidden" name="supplier_id" id="supplierId" value="{{ old('supplier_id', $purchase->supplier_id) }}">
                <input class="lookup-input" id="supplierDisplay" value="{{ old('supplier_id') ? $purchase->supplier?->company_name : 'Click to select supplier' }}" readonly required data-supplier-open>
            </label>
            <label>Status
            ...
            <label>Discount <input type="number" step="0.01" min="0" name="discount_amount" id="discountAmount" value="{{ old('discount_amount', $purchase->discount_amount ?? 0) }}"></label>
            <label class="full"><span><input type="checkbox" name="is_government_tax_collector" id="isGovernmentTaxCollector" value="1" style="width:auto" {{ old('is_government_tax_collector', $purchase->is_government_tax_collector) ? 'checked' : '' }}> Buyer is Government / BUMN PPh 22 collector</span></label>
        </div>
                    ...
                    @forelse ($purchase->items as $item)
                        <tr>
                            <td>
                                ...
                                <input type="hidden" name="product_label[]" class="product-label-hidden" value="{{ $item->product?->sku }} - {{ $item->product?->name }}">
                            ...
                            <td>
                                <select name="item_discount_type[]" class="line-discount-type">
                                    <option value="none" {{ $item->discount_type === 'none' ? 'selected' : '' }}>No Disc</option>
                                    <option value="fixed" {{ $item->discount_type === 'fixed' ? 'selected' : '' }}>Nominal</option>
                                    <option value="percentage" {{ $item->discount_type === 'percentage' ? 'selected' : '' }}>Percent</option>
                                </select>
                            </td>
                            <td><input type="number" step="0.01" min="0" name="item_discount_value[]" class="line-discount-value" value="{{ $item->discount_value ?? 0 }}"></td>
                            <td class="line-subtotal">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</td>
        ...
        <div class="totals">
            <div><strong>Subtotal</strong><span id="subtotalText">Rp {{ number_format($purchase->total_amount, 0, ',', '.') }}</span></div>
            <div><strong>Discount</strong><span id="discountText">Rp {{ number_format($purchase->discount_amount ?? 0, 0, ',', '.') }}</span></div>
            <div><strong>PPN</strong><span id="ppnText">Rp {{ number_format($purchase->ppn_amount, 0, ',', '.') }} ({{ $purchase->ppn_percentage }}%)</span></div>
            <div><strong>PPh / Tax Potong</strong><span id="withholdingText">Rp {{ number_format($purchase->withholding_tax_amount ?? 0, 0, ',', '.') }} ({{ $purchase->withholding_tax_name ?? 'No PPh' }})</span></div>
            <div><strong>DPP Barang / Jasa</strong><span id="dppSplitText">Rp {{ number_format($purchase->dpp_goods_amount ?? 0, 0, ',', '.') }} / Rp {{ number_format($purchase->dpp_services_amount ?? 0, 0, ',', '.') }}</span></div>
            <div><strong>Grand Total</strong><span id="grandText">Rp {{ number_format($purchase->grand_total ?? $purchase->total_amount, 0, ',', '.') }}</span></div>
        </div>

        <div class="row-actions" style="margin-top:16px">
            <button class="btn">Update Purchase</button>
            <a class="btn secondary" href="{{ route('purchases.show', $purchase) }}">Cancel</a>
        </div>
    </form>
</section>
...
<script>
    (function () {
        ...
        let lastPage = 1;
        let debounce;
        let supplierData = { ppn: parseFloat('{{ $purchase->supplier?->ppn_percentage ?? 0 }}'), entityType: '{{ $purchase->supplier?->entity_type ?? 'corporate' }}', hasNpwp: {{ $purchase->supplier?->tax_id_npwp ? 'true' : 'false' }}, pph21: parseFloat('{{ $purchase->supplier?->pph21_percentage ?? 5 }}') };
